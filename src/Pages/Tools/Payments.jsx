...
const Payments = () => {
    ...
    const btns = ['All payments', 'Paid', 'Pending']

    const {setDetail,setViewDetail} = useContext(MenuContext)
  ...
  return (
    <div className='font-euclid w-[70vw] mt-40 mx-auto'>
        <div className=" text-black text-[26px] pt-28 pb-20 font-semibold tracking-tight">Payments manager</div>
                    ...
                    return(
                        <div key={i} className='w-full grid grid-cols-6 items-center border border-zinc-400 py-2 pl-6 text-neutral-600 text-sm '>
                            ...
                            <span className='flex items-center gap-8'>
                                <div
                                className={`${item.p_status == 'Pending'?'bg-amber-500  border-amber-500 text-amber-500':
                                item.p_status == 'Paid'?'bg-emerald-600 border-emerald-600 text-emerald-600':null}
                                 ...
                                 className='cursor-pointer'/>
                                    ...
                                    className={`${toggle[i]?'flex':'hidden'} absolute top-0 left-0 w-28 z-50 bg-white rounded-xl shadow text-xs p-1 flex-col gap-1 font-[inter]`}>
                                        <span
                                         ...
                                         className='flex gap-2 items-center cursor-pointer text-black hover:bg-opacity-30 hover:bg-zinc-300'>
